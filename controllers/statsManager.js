    ...
    try {
        ...
        const [generalStats] = await pool.execute(`
            SELECT 
                ...
                (SELECT COUNT(*) FROM candidates WHERE statut = 'APPROUVE') AS total_candidates,
                (SELECT COUNT(DISTINCT userId) FROM votes) AS voters,
                (SELECT COUNT(*) FROM etudiants e JOIN users u ON e.userId = u.id WHERE u.actif = TRUE) AS total_students,
                CASE 
                    WHEN (SELECT COUNT(*) FROM etudiants e JOIN users u ON e.userId = u.id WHERE u.actif = TRUE) > 0 
                    THEN ROUND(((SELECT COUNT(DISTINCT userId) FROM votes) * 100.0 / 
                          (SELECT COUNT(*) FROM etudiants e JOIN users u ON e.userId = u.id WHERE u.actif = TRUE)), 2)
                    ELSE 0 
                END AS participation_rate
        `);

        const [votesEvolution] = await pool.execute(`
            SELECT 
                DATE(createdAt) AS vote_date,
                COUNT(*) AS vote_count
            FROM votes
            WHERE createdAt >= DATE_SUB(NOW(), INTERVAL ? DAY)
        ...
        if (format === 'json') {
            ...
            res.setHeader('Content-Disposition', 'attachment; filename="election-stats.json"');
            res.json(exportData);
        } else {
